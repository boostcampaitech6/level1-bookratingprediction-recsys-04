...
def main(args):
    ...
    config = {
        "model" : args.model,
        "use_clean_img" : args.use_clean_img,
    ...
    wandb.init(id = wandb_id, resume = "allow", project= args.project, name = f'{args.model}_{args.timestamp}', config = config, entity=args.entity)
    ...
    submission.to_csv(f'{args.saved_model_path}/{args.timestamp}/{args.timestamp}_{args.model}.csv', index=False)
...
if __name__ == "__main__":
    ...
    arg('--entity', type=str, default='uhhyunjoo', help='Username 이나 Team name 을 설정할 수 있습니다.')

    ############### EDA OPTION
    arg('--use_clean_img', type=bool, default=False, help= 'clean_books.csv 파일 사용 여부를 설정할 수 있습니다.')
    arg('--img_resize', type=int, default = 32, help= '이미지 전처리 시 resize 할 크기를 설정할 수 있습니다.')
    ...
    arg('--cnn_fm_ver', type = int, default = 0, help = 'CNN_FM 버전')
