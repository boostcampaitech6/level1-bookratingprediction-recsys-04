...
def image_vector(args, path):
    ...
    scale = transforms.RandomResizedCrop((args.img_resize, args.img_resize))
...
def make_clean_books(args, books):
    ...
    clean_df_.to_csv(f'{args.save_eda_path}/csv/clean_books_size.csv', index = False)
    
    new_df = clean_df_.drop(['img_width', 'img_height', 'img_path_tmp'], axis=1).copy()
...
def image_data_load(args):
    ...
    print('>>>>> clean 값: ', args.use_clean_img)
    if args.use_clean_img:
        print('>>>>>>> clean 사용 O')
    ...
    img_train = process_img_data(args, train, books, user2idx, isbn2idx, train = True)
    img_test = process_img_data(args, test, books, user2idx, isbn2idx, train = False)
