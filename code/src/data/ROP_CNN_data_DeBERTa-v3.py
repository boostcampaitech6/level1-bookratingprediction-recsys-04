...
def text_preprocessing(summary):
    ...
    ----------
    """
    summary = html.unescape(summary)    # html 코드 변환
    summary = re.sub("\s+", " ", summary)    # 하나 이상의 연속되는 공백 문자(\s+) 제거
    summary = summary.strip()    # 앞뒤 공백 제거
    summary = summary.lower()    # 소문자로 변경
    # summary = re.sub("[.,\'\"''""!?]", "", summary)    # 특수문자 제거
    # summary = re.sub("[^0-9a-zA-Z\\s]", " ", summary)    # 숫자[0-9], 알파벳 대소문자[a-zA-Z], 공백 문자[\s] 제외한 모든 문자 제거
    # 불용어 제거
...
def text_to_vector(text, tokenizer, model, device):
    ...
    for sent in tokenize.sent_tokenize(text):
        inputs = tokenizer(sent, return_tensors='pt')    # 토크나이징 (PyTorch 형태로 출력)
        with torch.no_grad():
            inputs = {k: v.to(device) for k, v in inputs.items()}    # 데이터에 GPU 할당 (inputs는 딕셔너리 형태)
            model = model.to(device)    # 모델에 GPU 할당
            outputs = model(**inputs)    # inputs가 딕셔너리 형태이므로 ** 사용
            encode_layers = outputs[0]
            sentence_embedding = torch.mean(encode_layers[0], dim=0)
    return sentence_embedding.cpu().detach().numpy()

# 전체 데이터셋의 text 데이터 처리 및 벡터화
def process_text_data(df, books, user2idx, isbn2idx, device, train=False, user_summary_merge_vector=False, item_summary_vector=False):
    ...
    df_['summary'].replace('', np.nan, inplace=True)
    df_['summary'].replace(' ', np.nan, inplace=True)
    df_['summary'].fillna(df_['book_title'], inplace=True)
    df_['summary_length'] = df_['summary'].apply(lambda x:len(x))

    # DeBERTa-v3로 사전학습 모델 변경
    ...
    if user_summary_merge_vector and item_summary_vector:
        ...
        #user_summary_merge_vector 생성
        user_summary_merge_vector_list = []
        for user in tqdm(df_['user_id'].unique()): #user summary -> vector
            vector = text_to_vector(summary_merge(df_, user, 5), tokenizer, model, device)
            user_summary_merge_vector_list.append(vector)
        user_review_text_df = pd.DataFrame(df_['user_id'].unique(), columns=['user_id']) #user_review_text_df : {user_id:user_summary_merge_vector}
        ...
        if train == True:
            np.save('./data/text_vector/train_user_summary_merge_vector_deberta.npy', vector)
        else:
            np.save('./data/text_vector/test_user_summary_merge_vector_deberta.npy', vector)
        '''
        #item_summary_vector 생성
        print('Create Item Summary Vector')
        item_summary_vector_list = []
        books_text_df = df_[['isbn', 'summary']].copy()
        books_text_df= books_text_df.drop_duplicates().reset_index(drop=True) #중복행 제거
        books_text_df['summary'].fillna('None', inplace=True) #결측치
        for summary in tqdm(books_text_df['summary']): #book summary to vector
        ...
        if not os.path.exists('./data/text_vector'):
            os.makedirs('./data/text_vector')
        if train == True:
            np.save('./data/text_vector/train_item_summary_vector_deberta.npy', vector)
        else:
            np.save('./data/text_vector/test_item_summary_vector_deberta.npy', vector)
    else:
        print('Check Vectorizer')
        print('Vector Load')
        if train == True:
            user = np.load('data/text_vector/train_user_summary_merge_vector_deberta.npy', allow_pickle=True)
        else:
            user = np.load('data/text_vector/test_user_summary_merge_vector_deberta.npy', allow_pickle=True)
        ...
        if train == True:
            item = np.load('data/text_vector/train_item_summary_vector_deberta.npy', allow_pickle=True)
        else:
            item = np.load('data/text_vector/test_item_summary_vector_deberta.npy', allow_pickle=True)
...
def text_data_load(args):
    ...
    ## users_items_rating_matrix
    
    # 'users'와 'books' 데이터프레임으로부터 사용자 ID와 아이템 ID를 가져옵니다.
    user_ids = users['user_id'].unique()
    item_ids = books['isbn'].unique()
    # 사용자 ID와 아이템 ID를 카테고리형 데이터로 변환하여 고유한 인덱스를 생성합니다.
    user_indices = pd.Series(range(len(user_ids)), index=user_ids)
    item_indices = pd.Series(range(len(item_ids)), index=item_ids)
    # 'rating' 데이터프레임으로부터 사용자 ID, 아이템 ID, 그리고 평점을 가져옵니다.
    rating_user_ids = train['user_id'].values
    rating_item_ids = train['isbn'].values
    ratings = train['rating'].values
    # 인덱스와 평점을 이용하여 `csr_matrix`를 생성합니다.
    # 사용자 ID와 아이템 ID가 'rating'에 없는 경우에는 인덱스를 -1로 설정합니다.
    users_items_rating_matrix = csr_matrix((ratings, 
                                (user_indices.get(rating_user_ids, -1), 
                                item_indices.get(rating_item_ids, -1))), 
                            shape=(len(user_ids), len(item_ids)))

    '''
    # 'users'에서 고유한 사용자 ID를 가져옵니다.
    unique_user_ids = users['user_id'].unique()
    # 'books'에서 고유한 ISBN을 가져옵니다.
    unique_isbns = books['isbn'].unique()
    # 'ratings' 데이터프레임에 존재하지 않는 사용자 ID와 ISBN을 위한 빈 데이터프레임을 만듭니다.
    empty_ratings = pd.DataFrame(index=unique_user_ids, columns=unique_isbns)
    # 'ratings' 데이터프레임으로 피벗 테이블을 만듭니다.
    users_items_rating_matrix = train.pivot_table 
    '''
    
    idx2user = {idx:id for idx, id in enumerate(ids)}  #DICT : idx -> user id 
    idx2isbn = {idx:isbn for idx, isbn in enumerate(isbns)}

    user2idx = {id:idx for idx, id in idx2user.items()}   #Dict : user ID -> idx
    isbn2idx = {isbn:idx for idx, isbn in idx2isbn.items()}

    train['user_id'] = train['user_id'].map(user2idx) ##train_ratings에서 idx로 변경 
    sub['user_id'] = sub['user_id'].map(user2idx)

    train['isbn'] = train['isbn'].map(isbn2idx)
    sub['isbn'] = sub['isbn'].map(isbn2idx)

    # 텍스트 데이터 처리를 거친 데이터 : 원본 훈련 데이터, user_summary_merge_vector, item_summary_vector
    ...
    data = {
            ...
            'text_test':text_test,
            'users_items_rating_matrix': users_items_rating_matrix
            }
    return data

#Data -> Train, Valid으로 Split
