...
def process_text_data(df, books, user2idx, isbn2idx, device, train=False, user_summary_merge_vector=False, item_summary_vector=False):
    ...
    df_ = pd.merge(df_, books_[['isbn', 'summary']], on='isbn', how='left') #train_rating(user id, isbn, rating)+ summary
    df_['summary'].fillna('None', inplace=True) #결측치 -> None
    df_['summary'] = df_['summary'].apply(lambda x:text_preprocessing(x)) #전처리(정규화)
    df_['summary'].replace({'':'None', ' ':'None'}, inplace=True) #결측치
    df_['summary_length'] = df_['summary'].apply(lambda x:len(x)) #summary 길이 열 추가

    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    model = BertModel.from_pretrained('bert-base-uncased').to(device)   #(사전학습된) Bert 모델 load
