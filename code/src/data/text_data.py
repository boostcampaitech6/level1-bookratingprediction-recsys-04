...
def summary_merge(df, user_id, max_summary):
    ...
    ----------
    """
    # 유저 별 summary 병합 (summary 길이가 긴 순서)
...
def process_text_data(df, books, user2idx, isbn2idx, device, train=False, user_summary_merge_vector=False, item_summary_vector=False):
    ...
    ----------
    df : pd.DataFrame
        기준이 되는 데이터 프레임을 입력합니다. train_ratings / test_ratings
    books :
        책 정보가 담긴 데이터 프레임을 입력합니다 pd.DataFrame.
    user2idx : Dict
    ...
    user_summary_merge_vector : bool
        user_summary_merge_vector를 사전에 전처리했는지에 대한 여부 입력합니다. (args.vector_create)
    item_summary_vector : bool
        item_summary_vector를 사전에 전처리했는지에 대한 여부 입력합니다.(args.vector_create)
    ----------
    """
    books_ = books.copy()
    ...
    if user_summary_merge_vector and item_summary_vector:
        ...
        books_text_df = df_[['isbn', 'book_title', 'summary']].copy()
        books_text_df= books_text_df.drop_duplicates().reset_index(drop=True)
        books_text_df['summary'].fillna(books_text_df['book_title'], inplace=True)
