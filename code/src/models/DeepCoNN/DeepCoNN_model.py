...
class DeepCoNN(nn.Module):
    def __init__(self, args, data):
        ...
        self.field_dims = np.array([len(data['user2idx']), len(data['isbn2idx'])], dtype=np.uint32)    # (user, item)
        self.embedding = FeaturesEmbedding(self.field_dims, args.deepconn_embed_dim)    # args.deepconn_embed_dim : user와 item의 임베딩 차원
        self.cnn_u = CNN_1D(
