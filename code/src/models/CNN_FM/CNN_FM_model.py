...
class FactorizationMachine(nn.Module):
    def __init__(self, input_dim, latent_dim):
        ...
        self.v = nn.Parameter(torch.rand(
            input_dim, latent_dim), requires_grad=True)
...
class FeaturesEmbedding(nn.Module):
    def __init__(self, field_dims: np.ndarray, embed_dim: int):
        ...
        self.offsets = np.array(
            (0, *np.cumsum(field_dims)[:-1]), dtype=np.int32)
...
class CNN_FM(torch.nn.Module):
    def __init__(self, args, data):
        ...
        self.field_dims = np.array(
            [len(data['user2idx']), len(data['isbn2idx'])], dtype=np.uint32)
    ...
    def forward(self, x):
        ...
        feature_vector = torch.cat([
            user_isbn_feature.view(-1, user_isbn_feature.size(1)
                                   * user_isbn_feature.size(2)),
            img_feature
        ], dim=1)
