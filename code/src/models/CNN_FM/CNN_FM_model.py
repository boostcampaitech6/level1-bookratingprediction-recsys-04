...
class CNN_Base(nn.Module):
    def __init__(self, args):
        ...
        self.img_size = args.img_resize
        self.cnn_latent_dim = args.cnn_latent_dim
        
        if self.img_size == 32:
            self.cnn_layer = nn.Sequential( # (3, 32, 32)
                                            nn.Conv2d(3, 6, kernel_size=3, stride=2, padding=1), # (6, 16, 16)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (6, 8, 8)
                                            nn.Conv2d(6, 12, kernel_size=3, stride=2, padding=1), # (12, 3, 3)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (12, 1, 1)
                                            )
        elif self.img_size == 64:
            
            if args.cnn_fm_ver == 1:
                self.cnn_layer = nn.Sequential( # (3, 64, 64)
                                            nn.Conv2d(3, 6, kernel_size=3, stride=2, padding=1), # (6, 32, 32)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (6, 16, 16)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (6, 8, 8)
                                            nn.Conv2d(6, 12, kernel_size=3, stride=2, padding=1), # (12, 3, 3)
                                            nn.ReLU(),
            ...
            elif args.cnn_fm_ver == 2:
                self.cnn_layer = nn.Sequential( # (3, 64, 64)
                                            nn.Conv2d(3, 6, kernel_size=3, stride=2, padding=1), # (6, 32, 32)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (6, 16, 16)
                                            nn.Conv2d(6, 12, kernel_size=3, stride=2, padding=1), # (12, 8, 8)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (12, 4, 4)
                                            nn.Conv2d(12, 12, kernel_size=3, stride=2, padding=1), # (12, 2, 2)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (12, 1, 1)
                                            )
            elif args.cnn_fm_ver == 3:
                self.cnn_layer = nn.Sequential(
                                            nn.Conv2d(3, 6, kernel_size=3, stride=2, padding=1), # (6, 32, 32)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=5, stride=2), # (6, 8, 8)
                                            nn.Conv2d(6, 12, kernel_size=3, stride=2, padding=1), # (12, 3, 3)
                                            nn.ReLU(),
                                            nn.MaxPool2d(kernel_size=3, stride=2), # (12, 1, 1)
                                            )
