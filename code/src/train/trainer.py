...
def train(args, model, dataloader, logger, setting):
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        if minimum_loss > valid_loss:
            ...
            os.makedirs(f'{args.saved_model_path}/{args.timestamp}', exist_ok=True)
            torch.save(model.state_dict(), f'{args.saved_model_path}/{args.timestamp}/{args.model}_model.pt')
    logger.close()
    return model
...
def test(args, model, dataloader, setting):
    ...
    if args.use_best_model == True:
        breakpoint()
        model.load_state_dict(torch.load(f'{args.saved_model_path}/{args.timestamp}/{args.model}_model.pt'))
    else:
        pass
    model.eval()
